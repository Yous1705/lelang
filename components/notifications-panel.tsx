      ...
      <div className="bg-white w-full md:w-96 h-screen md:h-auto md:max-h-96 rounded-t-lg md:rounded-b-lg shadow-lg flex flex-col">
        <div className="flex justify-between items-center p-4 border-b border-border">
          <h2 className="text-lg font-bold text-foreground">
            Notifications {unreadCount > 0 && <span className="text-sm text-primary">({unreadCount})</span>}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="p-4 text-center text-foreground-secondary">No notifications</div>
          ) : (
            <div className="divide-y divide-border">
              {notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`p-4 cursor-pointer hover:bg-surface transition ${!notification.read ? "bg-primary/5" : ""}`}
